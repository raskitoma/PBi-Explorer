...
tmp=$(mktemp ./.env.XXXXXX)
{
  ...
  for k in "${!CFG[@]}"; do
    v="${CFG[$k]}"
    # Values must be single-quoted so docker compose does NOT interpolate
    # any '$' characters inside them (e.g. inside the PBKDF2 password hash).
    # Single quotes also stop python-dotenv from doing escape processing.
    if [[ "$v" == *"'"* ]]; then
      err "value for $k contains a single quote — cannot safely encode for .env"
      exit 1
    fi
    printf "%s='%s'\n" "$k" "$v"
  done | sort
} > "$tmp"
# Mode 644 (not 600) so the non-root container user (uid 10001) can read
# the file through the volume mount. The .env is also single-quoted, so
# its contents are still inert to docker compose interpolation.
# Restrict the parent directory at the OS level — that's the real boundary.
chmod 644 "$tmp"
mv "$tmp" "$ENV_FILE"
ok ".env written (mode 644, values single-quoted)"
